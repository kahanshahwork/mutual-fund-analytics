 ...
 * ENGINE 2: SYNC NAV
 * ─────────────────────────────────────────────────────────────────
 * Run: npm run mf:nav           ← full history (first time, ~2-3 hrs)
 *      npm run mf:nav:today     ← only today's NAV (daily, ~10 min)
 *
 * Uses the SAME curated scheme codes as sync_schemes.ts — no DB query
 * to get the list, so no Supabase 1,000 row limit issue.
 *
 * Smart incremental: checks latest stored date per scheme, only
 * inserts rows newer than what's already in the DB.
 * Running twice same day = 0 new inserts. Safe to re-run anytime.
 */
...
// ── Same curated list as sync_schemes.ts ──────────────────────────────────────
// Importing directly here avoids any DB query for scheme codes.
...
async function syncScheme(
  ...
  try {
    ...
    const allRows: { scheme_code: number; nav_date: string; nav: number }[] = json.data
      .map((r: any) => ({ scheme_code: code, nav_date: parseDate(r.date), nav: parseFloat(r.nav) }))
      .filter((r: any) => r.nav > 0)

    if (!allRows.length) return { status: 'no_new', newNavs: 0 }

    // Only rows newer than what's already stored
...
async function main() {
  ...
  console.log(`  Schemes  : ${ALL_CODES.length} (curated list — no DB query)`)
  ...
  // Load latest stored NAV date per scheme — in pages to avoid 1000 limit
  console.log('  Loading latest stored dates per scheme...')
  const latestMap: Record<number, string> = {}

  // Paginate through nav_history to get max date per scheme
  // More efficient: query per batch of scheme codes
  const BATCH = 200
  for (let i = 0; i < ALL_CODES.length; i += BATCH) {
    const batch = ALL_CODES.slice(i, i + BATCH)
    const { data } = await supabase
      .from('nav_history')
      .select('scheme_code, nav_date')
      .in('scheme_code', batch)
      .order('nav_date', { ascending: false })
      .limit(BATCH * 5000)   // enough to cover all dates per scheme in batch
  ...
  console.log(`  Existing data  : ${Object.keys(latestMap).length} schemes already have NAV\n`)
  ...
  for (let i = 0; i < ALL_CODES.length; i++) {
    ...
    process.stdout.write(`\r  ${progressBar(i + 1, ALL_CODES.length)}  +${totalNavRows.toLocaleString()} rows`)
    await sleep(DELAY)
  }
